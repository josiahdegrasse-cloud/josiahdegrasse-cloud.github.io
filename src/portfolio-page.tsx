...
import { PortfolioRecruiterView } from "./portfolio-recruiter-view";
...
export function PortfolioPage() {
  ...
  const recruiterProject = pathname.match(/^\/portfolio\/projects\/([^/]+)$/)?.[1];
  const recruiterMode = pathname === "/portfolio/case-studies" || Boolean(recruiterProject);
  const missionQuery = new URLSearchParams(window.location.search).get("mission");
  const mission = missionQuery === "nfi-evidence-review" || missionQuery === "redhat-trust-review"
    ? missionQuery
    : null;

  const world = new URLSearchParams(window.location.search).get("world");

  if (recruiterMode) return <PortfolioRecruiterView projectId={recruiterProject} />;
...
function DirectMissionExperience({ mission }: { mission: DirectMission }) {
  ...
  const exit = () => {
    window.location.href = mission === "nfi-evidence-review"
    ...
    return (
      <main className="pqm pqm-direct-complete">
        <section>
          ...
          <p>
            Return to the case study for the product decisions, architecture, and outcome behind this interaction.
          </p>
          <button type="button" onClick={exit}>Return to the case study</button>
        </section>
      </main>
    );
  }
