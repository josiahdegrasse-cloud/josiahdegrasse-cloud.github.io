...
NFI images are genuine interface captures with synthetic demonstration data. The current NFI cover pairs decision review with a sensory radar chart for the coconut cheddar prototypes. HeadTap leads with concert discovery cards, supported by its orange music profile. Red Hat leads with the diagnostic checklist. Sample metrics are not project outcomes. Red Hat uses the team's actual Figma Make prototypes and original report artifacts, with research methods, iteration history, and individual team roles supported by the final report and presentation. Removed game source remains recoverable in Git history.
