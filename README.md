...
NFI images are genuine interface captures with synthetic demonstration data. NFI uses Josiah’s supplied sign-in screen with a freshly captured sensory spider chart. HeadTap pairs Josiah’s supplied landing-page image with freshly captured concert recommendations. Red Hat leads with the diagnostic checklist. Sample metrics are not project outcomes. Red Hat uses the team's actual Figma Make prototypes and original report artifacts, with research methods, iteration history, and individual team roles supported by the final report and presentation. Removed game source remains recoverable in Git history.
